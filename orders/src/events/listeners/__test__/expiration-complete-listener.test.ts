import { ExpirationCompleteEvent, OrderStatus } from '@sharedlib/common'
...
const setup = async () => {
    ...
    await ticket.save()
    const order = Order.build({ status: OrderStatus.Created, userId: 'asdfasdf', expiresAt: new Date(), ticket })
    await order.save()
